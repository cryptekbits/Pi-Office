...
- [x] BUG-019: Deferred Office tools, batch writes, anchors, and companion MCP handles drifted after Word API expansion
  - Category: Bug
  - Status: done
  - Priority: P0
  - Source: 2026-04-27 multi-agent implementation review of completed `FEATURE-028` through `FEATURE-030` and Word `FEATURE-009` through `FEATURE-024` slices.
  - Details: Review found that many deferred Word tools were discoverable but dispatched action types the Word executor did not handle; `office_batch_execute` was categorized as read and trusted model-supplied `confirmed`/`reviewRequired` flags for writes; deferred Office search/get could expose specialized tools without a callable runtime path; `word_collab_guard` was read-only but still had a document unprotect path; newer Word anchor metadata was stripped by `toAnchor`; and companion-routed MCP calls bypassed result handles.
  - Dependencies: FEATURE-028, FEATURE-029, FEATURE-030, FEATURE-017, FEATURE-024.
  - Subtasks:
    - [x] Add a write-doc `office_tool_call` dispatcher for discovered Office tools while keeping `office_execute_js` blocked and reviewable Word proposals on their normal review path.
    - [x] Align Word bridge action names and option shapes with implemented `applyWordAction` handlers across formatting, lists, tables, fields, content controls, annotations, proofing, protection, bookmarks, and hyperlinks.
    - [x] Classify `office_batch_execute` as write-doc and remove model-authored approval flags as a write bypass.
    - [x] Make `word_collab_guard` diagnostics-only and preserve durable anchor metadata including search result IDs/indexes, note body/reference target, bookmark names, and hyperlink IDs/addresses.
    - [x] Share MCP result-handle storage between taskpane and companion paths and add companion get/summarize/clear routes.
    - [x] Add regression coverage for bridge-to-executor action contracts, anchor metadata preservation, batch write gating, deferred dispatcher publication, and browser/companion MCP handle routing.
  - Acceptance Criteria:
    - [x] A discovered supported Office tool has an executable path without publishing every specialized schema in the baseline tool list.
    - [x] Batch document writes cannot be authorized by `confirmed` or `reviewRequired` fields supplied by the model.
    - [x] Read-only collaboration/protection diagnostics cannot mutate document protection.
    - [x] Durable Word anchors survive bridge normalization and route to deterministic target resolvers where supported.
...
- [ ] FEATURE-006: Add advanced mode where companion owns inference, providers, MCP, memory, and non-Office tools
  ...
  - Subtasks:
    - [x] Define a versioned taskpane-companion protocol for capabilities, settings sync, chat streaming, tool requests, Office tool execution, and auth migration.
    - [x] Add a persisted Basic/Smart Auto/Advanced runtime mode switch that affects model-visible companion capabilities.
    - [x] Sync non-secret taskpane preferences plus enabled provider/model selections into connected companion sessions.
    - [x] Add companion-side provider API-key storage with explicit user-action gating and secure storage where available.
...
- [x] FEATURE-028: Add deferred Office and MCP tool discovery with a canonical capability registry
  ...
  - Notes/Evidence: Claude's tool-search guidance describes deferred loading to reduce tool-definition context and improve selection accuracy for large toolsets. Pi-Office has growing Word API tasks plus a provenance-driven connector catalog, so deferred tool discovery should become a foundation before adding many more schemas. Closed 2026-04-27 by adding protocol-level capability search/detail types, Office registry metadata, `office_tool_search`, `office_tool_get`, compact baseline runtime publication through core Office tool definitions, browser-direct MCP search, companion MCP search route/client plumbing, and prompt guidance for deferred discovery. Regression coverage in `deferred-tool-discovery.test.ts` and `shared-tool-contracts-parity.test.ts` verifies search ranking/host gating, compact Word baseline publication, permission category preservation, runtime registry contracts, and MCP discovery wiring. 2026-04-27 review follow-up under `BUG-019` added the write-doc `office_tool_call` dispatcher so deferred Office tools are not merely discoverable: supported discovered tools can execute through the same host gating, capability checks, and reviewable Word proposal path while `office_execute_js` remains a direct manual escape hatch only. Validation: `npm run typecheck:addin`, `npm run typecheck:companion`, `npm run test:office`, and `git diff --check`; `BUG-019` follow-up validation passed `npm run typecheck:addin`, `npm run typecheck:companion`, `npm run test:office` (213 tests), `npm run build`, `npm run check:bundle`, and `npm run validate:manifests`.
...
- [x] FEATURE-029: Add structured batch execution for Office tools and MCP connectors
  ...
  - Notes/Evidence: Programmatic tool calling reduces latency and context by running repeated calls and filtering before results reach the model. Pi-Office should borrow that shape with typed taskpane/companion executors instead of giving the model raw unrestricted code over Word or MCP servers. Closed 2026-04-27 by adding typed structured batch protocol contracts, `office_batch_execute` and `mcp_batch_execute` tools, a taskpane batch executor that rejects raw `office_execute_js`, applies read/write step policy, and returns compact summaries with per-step outcomes, plus regression tests in `batch-execution.test.ts`. 2026-04-27 `BUG-019` follow-up moved `office_batch_execute` to the `write-doc` category and removed model-supplied `confirmed` / `reviewRequired` flags as permission evidence, so the batch envelope follows the normal write-doc approval/autonomy path and raw Office.js remains blocked inside batches. MCP call batching is implemented for the taskpane runtime path over existing browser-direct/companion MCP execution callbacks; connector-local result handles are covered by `FEATURE-030`. Validation: `npm run typecheck:addin`, `npm run typecheck:companion`, `npm run test:office` (160 tests), and `git diff --check`; `BUG-019` follow-up validation passed `npm run typecheck:addin`, `npm run typecheck:companion`, `npm run test:office` (213 tests), `npm run build`, `npm run check:bundle`, and `npm run validate:manifests`.
...
- [x] FEATURE-032: Add Word-native LaTeX and OfficeMath equation insertion
  - Category: Feature
  - Status: done
  - Priority: P1
  ...
  - Subtasks:
    ...
    - [x] Add matrix support and optional equation numbering/caption metadata.
    - [x] Add MathML input or a fuller audited conversion pipeline if Pi-Office needs equation coverage beyond the first-party LaTeX subset.
  ...
  - Acceptance Criteria:
    - [x] `word_equation` inserts editable native Word equations, not images and not literal `$$` text, for the supported LaTeX subset.
    - [x] Verification returns native math evidence including count, target, placement, display/inline mode, and any conversion warnings.
    - [x] Unsupported LaTeX commands fail with actionable correction guidance or a clearly labeled fallback, without claiming native success.
    - [x] Research-paper workflows can add equations while preserving surrounding paragraph flow and avoiding glued equation/prose paragraphs.
    - [x] Dependency choices and license/provenance decisions are documented before implementation lands.
  - Notes/Evidence: First implementation slice added `word_equation` with a first-party lightweight LaTeX-to-OMML subset, avoiding new dependency/license risk while preserving the researched dependency notes for a future fuller converter. The tool accepts LaTeX, `display`, `target`, and `placement`, inserts OfficeMath through `Range.insertOoxml` / `Body.insertOoxml`, returns persisted `m:oMath` verification counts, and reports unsupported-command warnings. Supported first slice covers fractions, square roots, sums/products/integrals as symbols with scripts, Greek letters, operators, and subscript/superscript forms, including the FFT formula shape from the Gemini run. 2026-04-28 follow-up added first-party `matrix` / `pmatrix` / `bmatrix` / `Bmatrix` / `vmatrix` / `Vmatrix` environment parsing into OMML matrix objects, optional visible block-equation numbering/caption fields, schema and prompt guidance for numbered/captioned equations, and regression coverage for matrix evidence plus inline metadata warnings. Final 2026-04-28 slice added first-party Presentation MathML input without adding a dependency: supported elements include `math`, `mrow`, `mi`/`mn`/`mo`/`mtext`, `mfrac`, `msqrt`, `msub`/`msup`/`msubsup`, `mfenced`, and `mtable`; unsupported MathML elements are preserved as plain OfficeMath text where possible with `mathml:*` warning metadata. `word_equation` now accepts `latex` or `mathml`, reports `requestedFormat`, `resolvedFormat: omml`, target/placement/display metadata, persisted `m:oMath` counts, and conversion warnings. Prompt guidance now forbids raw LaTeX or raw `<math>` MathML through `insertHtml`, and document-end equation insertion uses the same OfficeMath OOXML path that preserves paragraph flow. Local code confirms chat-only math rendering via `ChatView.tsx` (`remark-math` + `rehype-katex`) and KaTeX CSS import in `main.tsx`. Microsoft Support states Word supports UnicodeMath and LaTeX linear formats through the equation UI, and Microsoft Support also states the modern Office equation editor uses OMML as the preferred equation format in Office files. Microsoft Learn documents `Word.Range.insertOoxml`, and OOXML math reference material identifies `m:oMathPara` and `m:oMath` as the document representation for mathematical text. NPM research found `latex-to-omml@2.1.1` (MIT, depends on `mathjax-node` and `mathml2omml`), `mathml2omml@0.5.0` (LGPL-3.0-or-later), `mathjax@4.1.1` (Apache-2.0), and `temml@0.13.2` (MIT). Validation for the 2026-04-28 follow-ups passed with `npm run typecheck:addin`, `npm --prefix addin run test:office -- word-equation-tools` (248 tests), plus the full root build/bundle/manifest/whitespace gates for the final slice.
...
- [x] IMPROVEMENT-009: Continue splitting large Office action executors by feature domain
  - Category: Improvement
  - Status: done
  - Priority: P2
  ...
  - Subtasks:
    ...
    - [x] Split Excel worksheet/range/table structure handlers from formatting/filter handlers.
    - [x] Split PowerPoint table handlers into a `powerpoint-actions/` domain module.
    - [x] Split PowerPoint OOXML/package handlers into `powerpoint-actions/` modules.
    - [x] Split PowerPoint chart/notes handlers into `powerpoint-actions/` modules.
    - [x] Split PowerPoint media/image handlers into `powerpoint-actions/` modules.
    - [x] Split PowerPoint slide-structure handlers into `powerpoint-actions/` modules.
    - [x] Add or extend drift tests so action-domain handlers stay reachable from the host executor facade.
  - Acceptance Criteria:
    - [x] `excel-actions.ts` and `powerpoint-actions.ts` become thin dispatch/facade files rather than primary implementation stores.
    - [x] Each extracted module owns a cohesive Office.js feature family with typed inputs/outputs or clear helper contracts.
    - [x] Existing Office tests, build, bundle, manifests, and diff whitespace checks remain green.
  - Notes/Evidence: After `IMPROVEMENT-003`, the remaining action files are still large enough to slow feature work, even though Excel formatting/table-filter helpers and the PowerPoint icon catalog have been extracted. 2026-04-28 first slice extracted Excel chart creation/update/XML extraction to `addin/apps/taskpane/src/lib/office/excel-actions/charts.ts` and PivotTable create/update/sort/refresh/configuration to `addin/apps/taskpane/src/lib/office/excel-actions/pivots.ts`, leaving `excel-actions.ts` as the dispatcher for those domains. 2026-04-28 follow-up extracted PowerPoint table creation/value/cell/row/column/merge/split handlers to `addin/apps/taskpane/src/lib/office/powerpoint-actions/tables.ts`, reduced `powerpoint-actions.ts` from 1602 to 1380 lines, and added a `shared-tool-contracts-parity` regression so table actions stay routed through the domain module. 2026-04-28 media follow-up extracted PowerPoint icon search/insertion, image copy, and inline picture insertion into `addin/apps/taskpane/src/lib/office/powerpoint-actions/media.ts`, reducing `powerpoint-actions.ts` to 1209 lines and adding a parity regression so media actions stay routed through the domain module. 2026-04-28 slide-structure follow-up extracted add/agenda/transition/combine/reorder/layout/move/duplicate/delete/select slide handlers into `addin/apps/taskpane/src/lib/office/powerpoint-actions/slide-structure.ts`, reduced `powerpoint-actions.ts` to 666 lines, and added a parity regression so slide-structure actions stay routed through the domain module. 2026-04-28 package/chart/notes follow-up extracted presentation package/theme, slide notes, slide-chart inspection, slide-note mutation, and slide-chart mutation routing into `addin/apps/taskpane/src/lib/office/powerpoint-actions/package.ts`, reducing `powerpoint-actions.ts` to 583 lines and adding a parity regression so package/notes/chart actions stay routed through the domain module. 2026-04-28 final Excel follow-up extracted range/value/formula/format/sort/filter/validation/conditional-format handlers to `addin/apps/taskpane/src/lib/office/excel-actions/ranges.ts`, worksheet lifecycle/visibility/print-area handlers to `addin/apps/taskpane/src/lib/office/excel-actions/worksheets.ts`, table create/style/filter handlers to `addin/apps/taskpane/src/lib/office/excel-actions/tables.ts`, and inline picture insertion to `addin/apps/taskpane/src/lib/office/excel-actions/media.ts`, reducing `excel-actions.ts` to a 70-line dispatcher. The shared tool-contract regression now asserts Excel range, worksheet, table, and media actions route through their focused domains. Validation passed with `npm run typecheck:addin`, `npm --prefix addin run test:office -- shared-tool-contracts-parity` (245 tests), `npm run build`, `npm run check:bundle` (`main.js=1533.9 KiB`), `npm run validate:manifests`, and `git diff --check`.

- [x] IMPROVEMENT-010: Update GitHub Actions workflow for Node 24 runner transition
  - Category: Improvement
  - Status: done
  - Priority: P2
  ...
  - Dependencies: None.
  - Subtasks:
    - [x] Check whether newer `actions/checkout` and `actions/setup-node` versions support Node 24.
    - [x] Update `.github/workflows/ci.yml` to Node-24-compatible actions or set the documented temporary compatibility flag with an expiration note.
    - [x] Run CI-equivalent local gates on Node 24 after the change and record the result.
  - Acceptance Criteria:
    - [x] CI no longer emits the Node.js 20 action deprecation annotation.
    - [x] Install, typecheck, build, bundle budget, manifest validation, and Office tests still pass on GitHub Actions or an equivalent Node 24 local gate before push.
  - Notes/Evidence: CI run `24986830664` passed, but emitted GitHub's Node.js 20 deprecation warning for `actions/checkout@v4` and `actions/setup-node@v4`. Closed 2026-04-28 by updating `.github/workflows/ci.yml` to `actions/checkout@v6`, `actions/setup-node@v6`, and `node-version: 24`. Official action docs/release notes checked on 2026-04-28: `actions/checkout` documents v6 usage and notes v5+ updated to the Node 24 runtime; `actions/setup-node` documents v6 usage with `node-version: 24`, Node 24 semver support, npm caching, and Node 20/22/24 matrix examples. The Office test harness now invokes Windows npm/npx commands without combining `shell: true` with an argument array, removing the Node 24 `[DEP0190]` warning. Local validation ran on Node `v24.13.0`; install/typecheck/build/bundle/manifest/test results are recorded in `AGENTS.md` Last worked on.
...
- [ ] IMPROVEMENT-004: Build a professional PowerPoint visual asset and icon pipeline
  ...
  - Subtasks:
    - [x] Define supported asset sources for icons, generated images, user-provided images, and reusable slide components.
    - [x] Replace glyph-textbox fallback with higher-quality Office-compatible icon/image insertion where possible.
    - [x] Add preview/verification support for inserted assets.
    - [x] Add tests or manual validation for icon insertion, generated-image insertion, and asset fallback behavior.
  - Acceptance Criteria:
    ...
    - [x] Asset insertion reports source, format, and fallback behavior.
    - [ ] At least one deck-quality asset workflow is validated in PowerPoint.
  - Notes/Evidence: Review pointed to the small runtime icon catalog and glyph-textbox insertion path in `addin/apps/taskpane/src/lib/office/powerpoint-actions.ts`, plus ChatGPT inspiration assets around generated slide stores and slide screenshots. 2026-04-28 first implementation slice replaced the built-in PowerPoint icon catalog entries with original SVG geometry, added SVG-to-PNG rasterization for default `insert_icon` insertion through `shapes.addImage`, reports source/format/fallback metadata, and gates glyph text-box fallback behind `allowGlyphFallback=true` or `fallback: "glyph-textbox"`. Prompt/package guidance now says built-in icons insert as SVG-rasterized image shapes where supported. Automated coverage verifies SVG-backed catalog assets, image-shape insertion preference, and explicit glyph fallback gating. 2026-04-29 second slice added `docs/powerpoint-asset-pipeline.md`, a typed supported asset-source catalog for built-in SVG icons, provided image payloads, copied PowerPoint shape snapshots, generated-image handoff, selected image-shape verification, and planned reusable components; `search_icons`, `insert_icon`, image insertion, and image-copy results now report source/format IDs where available; `verify_slide_visual` reports selected asset verification metadata without claiming source provenance for generic image shapes. 2026-04-29 generated-image orchestration slice now tags `generate_image` and generated-image-card insertions with `generated-image-base64`, lets `insert_slide_element`/`copy_image_between_slides` carry direct image asset source IDs, applies PowerPoint image shape alt/name metadata during insertion, and lets `verify_slide_visual` infer generated-image provenance from Pi-Office alt text. 2026-04-29 reusable-component slice adds native-shape `metric-card`, `quote-callout`, and `section-divider` components through `insert_slide_element` / `add_reusable_component`, advertises the component catalog from `search_icons`, tags component shapes with Pi-Office alt metadata for selected-shape verification, updates prompt/schema/docs guidance, and lazy-loads PowerPoint media/component domains so the production main chunk stays below Vite's large-chunk warning line. 2026-04-29 asset-preview slice adds lightweight `assetPreview` descriptors to icon search/insert results, image insertion/copy results, and reusable component insertion results without base64 payloads; automated coverage now guards icon, image, and component preview descriptors plus docs/source wiring. 2026-04-29 validation-planning slice added the `powerpoint-asset-pipeline-deck-quality` manual smoke scenario to `addin/scripts/run-office-smoke.mjs`, documented the canonical deck-quality validation workflow in `docs/powerpoint-asset-pipeline.md`, and added regression coverage so the smoke plan keeps exercising `search_icons`, `insert_icon`, generated-image insertion, reusable components, and `verify_slide_visual`. Validation passed: `npm --prefix addin run test:office -- powerpoint-verify-media-tools` (278 tests), `npm --prefix addin run smoke:office:powerpoint -- --list`, `npm run typecheck:addin`, `npm run build`, `npm run check:bundle` (`main.js=1554.6 KiB`), `npm run validate:manifests`, and `git diff --check`. Remaining scope: run the real PowerPoint desktop deck-quality/manual validation pass and record the dated result.
