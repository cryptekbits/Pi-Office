...
- `BUG-004` has automated hardening in place: Office state refreshes are latest-only per session and repeated transient refresh failures are deduped; automated release gates passed, with real Word desktop smoke still tracked as the remaining closure gate.
