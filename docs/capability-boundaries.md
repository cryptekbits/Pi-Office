...
Connector **configuration** (including user-pasted secrets and static HTTP headers) is saved in the taskpane's encrypted `localStorage` envelope on the device. Local `stdio`, local HTTP, and profiles marked `requiresCompanion` verify and execute through the optional companion; browser-direct hosted HTTP profiles can verify from the taskpane only when their catalog profile permits it and provider CORS allows it. Companion-brokered OAuth profiles, such as Granola, use the companion for system-browser sign-in, callback handling, token exchange, and MCP execution. Users can save configuration first and run **Check connection** when the required runtime is online. See `docs/privacy-and-storage.md` for storage boundaries.
