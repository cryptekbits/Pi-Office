...
- Connector setup facts now live in the manifest `addin/packages/pi-office-pack/src/connector-catalog.yaml` plus per-connector files under `addin/packages/pi-office-pack/src/connector-catalog/`; the generated TypeScript catalog is checked by `npm --prefix addin run check:connector-catalog`.
