...
- Granola remains an official hosted OAuth MCP, but live Office webviews can hit provider CORS at DCR/token exchange. Pi-Office should fail closed with OAuth broker or companion-mediated sign-in guidance rather than showing raw browser CORS errors.
...
- System-browser OAuth is intentionally not enabled by swapping the popup API alone. `Office.context.ui.openBrowserWindow` opens an external browser window, but connector OAuth needs a verified state/token handoff back to the taskpane; `FEATURE-019` tracks a companion or hosted broker path for Granola-style CORS-blocked DCR and true system-browser sign-in.
