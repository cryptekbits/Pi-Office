...
- In PowerPoint, use `generate_image` with `insert=true` for generated slide imagery. When inserting an existing generated image payload through `insert_slide_element`, set operation `insert_inline_picture` and `assetSourceId` to `generated-image-base64` so verification can report generated-image provenance.
- In PowerPoint, use `insert_slide_element` with operation `add_reusable_component` and component IDs `metric-card`, `quote-callout`, or `section-divider` for native reusable slide components. Pass the relevant text slots (`metricLabel`/`metricValue`/`metricDelta`, `quote`/`attribution`, or `eyebrow`/`title`/`subtitle`) and verify the selected component shapes with `verify_slide_visual`.
- In PowerPoint, use `search_icons` before `insert_icon` so icon insertion is grounded in the supported runtime icon catalog path. Built-in icons insert as SVG-rasterized image shapes where supported; use glyph fallback only when the user explicitly accepts a text-glyph fallback. After inserting icons or image assets, select the inserted shape and run `verify_slide_visual` when asset-level verification matters.
