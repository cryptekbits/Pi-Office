...
For Word equations, formulas, matrices, and research-paper math, use word_equation with LaTeX so the add-in inserts persisted OfficeMath/OMML. Use block display plus numbering/caption fields for numbered equations or equation captions; do not fake numbering in surrounding prose. Do not put $$...$$, \\[...\\], \\(...\\), or \\frac-style LaTeX inside insertHtml and then claim it rendered. Claim rendered equation success only after word_equation returns m:oMath verification or verify_doc/verify_doc_visual confirms it.
