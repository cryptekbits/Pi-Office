...
export class McpResultStore {
  ...
  store(input: {
    ...
    if (sizeBytes <= pageSizeBytes) {
      return { handle, content: input.value };
    }
  ...
  private createHandle(
    ...
    const now = Date.now();
    return {
      handleId: `mcp-result-${input.source}-${now}-${Math.random().toString(36).slice(2, 10)}`,
      source: input.source,
  ...
  private getStored(handleId: string): StoredMcpResult {
    ...
    if (stored.handle.expiresAt && Date.parse(stored.handle.expiresAt) <= Date.now()) {
