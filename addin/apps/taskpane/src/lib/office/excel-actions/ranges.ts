...
import { resolveExcelRange, resolveExcelTable, resolveExcelWorksheet } from "../excel-targets";
import { applyExcelRangeBorders } from "./formatting";

export function isExcelRangeAction(type: string): boolean {
  return [
    "insertText",
    "setRangeValues",
    "setRangeFormulas",
...
export function applyExcelRangeFormatting(range: Excel.Range, action: OfficeHostAction): void {
...
export async function applyExcelRangeAction(
  context: Excel.RequestContext,
  ...
  throw new Error(`Unsupported Excel range action: ${type}`);
}
