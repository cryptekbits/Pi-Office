...
export const WORD_OFFICE_TOOL_DEFINITIONS: readonly OfficeToolDefinition[] = [
  ...
  },
  {
    ...
    parameters: Type.Object({
      ...
      allowDuplicatePageBreak: Type.Optional(Type.Boolean({ description: "Defaults false for page breaks. Set true only when an additional adjacent page break is intentional." })),
    }),
    executor: "office-bridge",
  },
  {
    ...
    label: "Word Equation",
    description:
      "Word-only native OfficeMath equation insertion from LaTeX or supported Presentation MathML. Use this instead of insertHtml for formulas, equations, and research-paper math.",
    discovery: {
      tier: "specialized",
      capabilityIds: ["word.equations", "word.officemath"],
      keywords: ["word", "equation", "formula", "latex", "mathml", "math", "omml", "officemath"],
      summary: "Convert LaTeX or supported MathML into persisted Word OfficeMath OOXML and verify m:oMath evidence.",
      riskLevel: "medium",
      requirementSets: [{ name: "WordApi", minVersion: "1.1" }],
      fallback: "If native OfficeMath insertion fails, explain the limitation; do not insert raw LaTeX/MathML and claim it rendered.",
    },
    parameters: Type.Object({
      latex: Type.Optional(Type.String({ description: "LaTeX math. Delimiters like $$...$$, \\[...\\], or \\(...\\) are accepted but not required. Provide latex or mathml." })),
      mathml: Type.Optional(Type.String({ description: "Presentation MathML <math> input for the supported first-party subset: mrow, mi/mn/mo/mtext, mfrac, msqrt, msub/msup/msubsup, mfenced, and mtable." })),
